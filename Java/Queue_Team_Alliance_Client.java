...
public class Queue_Team_Alliance_Client {
    String[] teams;
    int team_spot_counter = 0;
    int alliance_spot_counter = 0;
    int length;

    public Queue_Team_Alliance_Client(int length) {
    ...
    public void menu() {
        int temp = Integer
                .parseInt(JOptionPane.showInputDialog("1] Add a team name" + "\n" + "2] Pick an alliance member" + "\n"
    ...
    public void allianceChoose() {
        ...
        if (alliance_spot_counter < length && teams[alliance_spot_counter] != null) {
            String output = "";
            for (int i = alliance_spot_counter; i < teams.length; i++) {
                if (teams[i] != null) {
                    output = output + teams[i] + "\n";
                }
            }
            String team = JOptionPane.showInputDialog(
                    teams[alliance_spot_counter] + ", who would you like to alliance with?" + "\n" + output);

            for (int i = 0; i < length; i++) {
                if (teams[i] != null && team != null) {
                    if (teams[i].equals(team)) {
                        JOptionPane.showMessageDialog(null,
                                teams[i] + " and " + teams[alliance_spot_counter] + " are alliance members.");
                        teams[i] = null;
